...
变更：+440 / -42 行，5 文件。基础理论层**净增 0 篇、升级 4 篇**（4 占位 → 4 实质，原 9 篇全已成型）。
...
vault 净变化：178 → 187 篇（净 +9：8 个 README + 1 个 log 增量），0 文件目录数：8 → 0。
...
## [2026-06-15] apply | D 线全套草稿 7 份落到正式位置

老大给方案 → 我列 4 项任务 + 起草 → 老大"全部应用"。

**应用 7 份草稿到正式位置**（`feat/d-line-setup-2026-06-15` 分支）：

- 2 个项目 → [[../05-Projects/Active/2026-06-repro-building-effective-agents/README|复现 Building effective agents]] / [[../05-Projects/Active/2026-06-mcp-tool-agent/README|MCP 工具调用 Agent]]
- 3 个索引 → [[../06-Resources/论文清单]] / [[../06-Resources/书单与课程]] / [[../06-Resources/awesome 清单汇总]]
- 1 个模板 → [[../Templates/tpl-Clipping]]
- 1 个笔记 → [[../03-Prompt/技术/不同模型 Prompt 差异]]

**反链更新**：
- [[../00-MOC/项目看板]] — 加 2 个项目
- [[../00-MOC/Agent 全景地图]] — 加 2 个项目
- [[../00-MOC/Prompt Engineering 地图]] — 加「不同模型 Prompt 差异」
- [[../00-MOC/框架与工具地图]] — 加 4 条 D 线资源索引入口
- [[../05-Projects/Active/README]] — 加 2 行项目列表
- [[Obsidian 知识库丰富方案]] — `status: 草稿 → 已审（§5 全部采纳）`

**清理**：删除 `Inbox/_drafts/{projects,resources,templates,prompts}/` 下 7 个已应用草稿。

**净变化**：vault 187 → **194 篇**（+7）+ 1 个模板。
05-Projects 0 实质 → 2 实质，06-Resources 多了 3 个内容型索引，03-Prompt 多了 1 篇跨模型差异，Templates 多了 1 个 Clipping 模板。

---

## [2026-06-15] refactor | 把 2 个使用说明/方案文件挪到 00-MOC/

老大说"使用说明之类的放在内部（06-Resources/）太影响结构了"。

**移动 2 个文件**（git rename，可追历史）：
- `06-Resources/Obsidian知识库丰富方案.md` → `00-MOC/Obsidian 知识库丰富方案.md`
- `06-Resources/RSS订阅清单 使用说明.md` → `00-MOC/RSS订阅清单 使用说明.md`

**OPML 不动**：`06-Resources/RSS订阅清单-AI-Agent.opml` 留 06-Resources/（是数据文件不是文档）。

**更新 7 处 wikilink**：
- 移动后 2 个文件里的内部链接（指向 OPML 改为 `../06-Resources/...`）
- 5 个外部引用文件（[[../00-MOC/框架与工具地图]] / [[../06-Resources/论文清单]] / [[../06-Resources/awesome 清单汇总]] / [[../06-Resources/书单与课程]] / 本 log）

**理由**：`00-MOC/` 已有同类（`Graph 视图配置.md` / `标签规范.md` 都是操作类文档），这 2 个新文件属于"KB 使用与策略"类，放一起更顺。

---

## [2026-06-15] chore | untrack .obsidian/graph.json（修 2026-06-13 半截 bug）

2026-06-13 老大说"`graph.json` 应被 gitignore"，当时加了 `.gitignore` 规则但**漏了 `git rm --cached`**——`gitignore` 对已 tracked 的文件无效，结果这个 per-device 状态文件 2 天来一直在产生噪音 commit。

**修正**：
- `git rm --cached .obsidian/graph.json`（本地文件保留，Obsidian 继续用）
- 修 [[KB 构建规范]] §九："保留跟踪的 .obsidian 文件"清单移除 `graph.json`

**验证**：本地 `ls -la .obsidian/graph.json` 仍存在（Obsidian 需要的），但 `git ls-files` 不再列出它。
